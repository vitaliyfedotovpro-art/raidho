...
### Added
- **Auto-distillation of procedures (opt-in).** A successful read-only tool-loop is captured into a deterministic procedure (deterministic data-collection steps + one generative synthesis step) and stored; the next similar task matches it and runs without the LLM loop — spend falls with repetition. Gates: opt-in (`Session(autodistill=True)` / `CODER_AUTODISTILL=1` / REPL `/learn`); successful LLM-path runs only; READ-ONLY only (allowlisted leading command, no mutation/redirect tokens, dual-use interpreters excluded — writes never auto-replay); 2..15 steps; a fail-closed LLM safety-verify pass; neutral fitness with auto-sink + LLM fallback on any procedure crash. New `agent/distill.py`. Verified live (DeepSeek): run 1 learned, run 2 replayed deterministically.
- **Persistent memory across runs.** `AgentMemory(path=...)` loads facts on start and `save()` writes them back; `Session` saves after any memory-mutating turn (code tool-loop, council) and the REPL saves on exit. Per-project by default — the CLI stores at `<workdir>/.raidho/memory` (a coder agent's facts belong to that codebase); override with `CODER_MEMORY=<path>` or disable with `CODER_MEMORY=off`. Corrupt/old files start empty rather than crashing. Verified live cross-process AND cross-lingual: run 1 stored a fact, a fresh process recalled it from a paraphrased query.
